...
benchinit() {
	...
	if  [ ! -e './speedtest-cli/speedtest' ]; then
        ...
        version=`curl -Is https://github.com/librespeed/speedtest-cli/releases/latest | grep location: | cut -d "/" -f 8 | tr '\r' ' '| cut -d " " -f 1`
        wget --no-check-certificate -qO speedtest.tar.gz https://github.com/librespeed/speedtest-cli/releases/download/${version}/librespeed-cli_${version:1}_linux_amd64.tar.gz
	fi
	mkdir -p speedtest-cli && tar zxvf speedtest.tar.gz -C ./speedtest-cli/ > /dev/null 2>&1 && chmod a+rx ./speedtest-cli/speedtest

	if  [ ! -e 'tools.py' ]; then
		echo "正在安装 tools.py ..."
		wget --no-check-certificate https://raw.githubusercontent.com/nivrrex/superspeed/master/tools.py > /dev/null 2>&1
	fi
	chmod a+rx tools.py
